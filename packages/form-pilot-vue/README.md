...
```bash
pnpm add @form-pilot/vue vue-demi
```
...
- 当你需要让 AI 操作表单时，结合 `@form-pilot/mcp-client` 使用：
...
> 业务应用不需要直接引用 `@form-pilot/core`；所有核心能力通过本包句柄与 MCP 客户端间接使用。
...
```bash
pnpm --filter @form-pilot/vue run typecheck
pnpm --filter @form-pilot/vue run build
```
