...
面向企业的前端 MCP 客户端，安全地让 AI 通过受控工具操作真实 Vue 表单。该包是 AI/MCP 与 `@fhuang/form-pilot-vue` 之间的安全桥梁，强制权限、字段级防护、提交确认，并对每一次 AI 操作进行结构化审计。
...
- 运行时
  ...
  - `@fhuang/form-pilot-vue`：唯一表单接入层（业务仅依赖此包与 MCP 客户端）
- 工作区内部
  - `@fhuang/form-pilot-core`：表单语义与适配器契约（不直接暴露给业务）
...
```bash
pnpm add @fhuang/form-pilot-mcp-client @fhuang/form-pilot-vue vue-demi
```
...
- 无 DOM 访问
- 表单操作全部通过 `@fhuang/form-pilot-vue` 的句柄方法委托，实现兼容 Vue2/Vue3
...
```bash
pnpm --filter @fhuang/form-pilot-mcp-client run typecheck
pnpm --filter @fhuang/form-pilot-mcp-client run build
```
...
- 不实现或暴露 `@fhuang/form-pilot-core` 细节，业务仅通过 `@fhuang/form-pilot-vue` 与本包交互
- 不硬编码任何 UI 库 API；兼容由适配器支持的表单实现
- 不实现 AI 决策或后端服务；本包仅执行受控工具调用

---

如需在你的表单系统中启用 AI 操作：先用 `@fhuang/form-pilot-vue` 将真实表单接入，再用 `MCPClient` 开启受控工具能力与审计。该方案在 Vue2/Vue3 下保持一致 API，满足企业级的权限与合规要求。